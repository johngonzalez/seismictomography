...
#include "MD4.h"       //Libreria para resolver modelo directo (Diferencias Finitas)
...
int main() {
  ...
  const char *ArchMod  ="DIR_MODI3-1";                             //Archivo con Modelo Inicial
  ...
  int      i,j,k,*NumBlo,NumBloT,NumSis,NumEst,NumFil,NumCol,NumCol2,NumPicT,NumEstC,iteracion,*Fecha;
  ...
  NumCol    =4*NumSis;                                                        //Calcula numero de columnas (incognitas) de la matriz sensibilidad
  NumCol2   =NumCol+NumBloT;
  ...
  Mod1      =new double [NumCol2];
  Mod2      =new double [NumCol2];
  ...
  //for(j=0;j<NumBloT;j++) cout<<Mod1[4*NumSis+j]<<"  ";
  // cout<<endl<<endl;
  ...
  //while(iteracion<1){
    ...
    cout<<iteracion<<"  "<<e1<<" "<<abs(De)<<endl<<endl;

...
int main() {
  ...
  const char *ArchPar ="CAL";                             //Archivo de parametros
  const char *ArchPic ="DAT";                             //Archivo de Datos
  const char *ArchEst="PosEst";                               //Archivo de Estaciones
  const char *ArchMod="MODI";                               //Archivo con Modelo Inicial
  const char *ArchSal="MODF";                          //Archivo Salida. Modelo Final
  ...
  int      i,j,k,NumBlo,NumSis,NumEst,NumFil,NumCol,NumPicT,NumEstC,iteracion,*Fecha;
  ...
  NumBlo    =Numero(ArchMod,1);                                               //Adquiere numero de bloques
  NumFil    =NumEstC*NumSis;                                                  //Calcula numero de filas (ecuaciones) de la matriz sensibilidad
  NumCol    =4*NumSis+NumBlo;                                                 //Calcula numero de columnas (incognitas) de la matriz sensibilidad
  ...
  Mod1   =ModeloInicial(ArchMod,NumSis);                                           //Adquiere hipocentros y modelo de velocidad inicial
  ...
  while(iteracion<1e30){
    ...
    //Animacion(Mod1,Pos,Tobs,NumPic,NumPicP,e1,NomEst,V);
  ...
  //AnimacionFinal(Mod1,Pos,Tobs,NumPic,NumPicP,e1,NomEst,V);
  ...
  if(e2<e1){
    ...
    for(j=0;j<NumBlo;j++) FILE<<Mod2[j]<<"  ";
    FILE<<endl;
  }
  else{
    FILE<<e1<<endl<<endl;

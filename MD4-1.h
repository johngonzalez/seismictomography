...
double *CalculeTA(int NEST,int *NumBlo,int NumSis,double *pos,double *Mod,string *Fase,double *Lim,double & TC){
  ...
  double vel;//velocidad de la capa
  vel=Mod[4];//velocidad de la capa
  double S0=0.04;
  double *T,*TTeo;
...
double * CalculeTT(int NumEstC,int *NumBlo,int NumSis,double *Pos,double *Mod,string *Fase, double *L,double & TC) {
  ...
  for(k=0;k<NumSis;k++) {
    ...
    tteoP=CalculeTF(NumEstC,NumBlo,NumSis,Pos,ModP,FaseP,L,TC);
...
double * CalculeG2(int NEST,int *NumBlo,int NumSis,double *pos,double *Mod,string *Fase,double *Lim, double & TC){
  ...
  for (k=0;k<NumSis;k++){  
	...
	if (j1==3)  {Mod1[j1]=Mod[4*k+j1];                     Mod2[j1]=Mod[4*k+j1];                    }
	if (j1>3)   {Mod1[j1]=Mod[4*(NumSis-1)+j1];            Mod2[j1]=Mod[4*(NumSis-1)+j1];           }
      }
      
      for(i=0;i<NEST;i++) FaseP[i]=Fase[i+NEST*k];
      
      if(j<3){
	tteo1=CalculeTF(NEST,NumBlo,NumSis,pos,Mod1,FaseP,Lim,TC);
	tteo2=CalculeTF(NEST,NumBlo,NumSis,pos,Mod2,FaseP,Lim,TC);
      }
	...
	if(j>3)  l=j-4; c[(NumSis*l+k)*NEST+i] =(tteo2[i]-tteo1[i])/DMod[j];
      }
